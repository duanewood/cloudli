...
const getCss = () => {
  if (!css) {
    const visualDiffCss = fs.readFileSync(
      path.join(__dirname, '/diff/html.css')
    )
    const annotatedDiffCss = fs.readFileSync(
      path.join(__dirname, '/diff/annotated.css')
    )
...
class DiffVisitor {
  constructor(basePath, htmlFilename) {
    ...
    if (htmlFilename) {
      ...
      this.writeStream = fs.createWriteStream(htmlFilename, {
        encoding: 'utf-8',
        flag: 'w'
      })
    ...
    if (!fs.existsSync(fullName)) {
      ...
      try {
        ...
        if (delta) {
          ...
          if (this.htmlFilename) {
            const visualDiff = jsondiffpatch.formatters.html.format(
              delta,
              fileDoc
            )
  ...
  async visitDir(dir) {
    ...
    return fs.readdir(dir).then(async files => {
      ...
      await Promise.all(
        jsonFiles.map(file => this.visitFile(path.join(dir, file)))
      )
      return Promise.all(
        subdirs.map(subdir => this.visitDir(path.join(dir, subdir)))
      )
    })
  }
