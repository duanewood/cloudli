...
async function createIndex(index, indexMappingFile, addAliases, skipExisting) {
  if (addAliases) {
    ...
    if (readIndices || writeIndices) {
      const readIndicesString = readIndices
        ? readIndices.map(i => `${index}_read -> ${i}`).join('\n') + '\n'
        : ''
      const writeIndicesString = writeIndices
        ? writeIndices.map(i => `${index}_write -> ${i}`).join('\n') + '\n'
        : ''
      throw new Error(
        `Unable to create index because aliases already exist:\n${readIndicesString +
          writeIndicesString}`
      )
    }
  }
  ...
  if (addAliases) {
    ...
    logger.info(
      Colors.info(
        `Created index ${chalk.bold(
          indexWithDateTime
        )} and aliases ${chalk.bold(readAlias)} and ${chalk.bold(writeAlias)}`
      )
    )
  } else {
