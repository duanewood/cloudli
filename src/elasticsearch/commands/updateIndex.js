  ...
  try {
    ...
    for (const indexConfig of indices) {
      if (!indexConfig.indexMapping) {
        throw new Error(
          `Missing indexMapping property from config for index ${indexConfig.name}`
        )
      }
    ...
    logger.info(
      Colors.prep(
        `About to create a new index and reload all objects for indices ${indicesMsg}.`
      )
    )
    const confirmed =
    ...
    if (confirmed) {
      logger.info(
        Colors.start(
          `Starting update index and reload documents for indices ${indicesMsg}`
        )
      )
      await updateIndexReload(indices, config, verbose)
      logger.info(
        Colors.complete(`Completed update index and reload documents.`)
      )
    }
  } catch (error) {
    ...
    indices.map(async indexConfig => {
      ...
      try {
        if (verbose) {
          logger.info(
            Colors.info(
              `Changing write alias ${chalk.bold(
                `${index}_write`
              )} from ${chalk.bold(`[${writeIndices}]`)} to ${chalk.bold(
                newIndex
              )}`
            )
          )
        ...
        try {
          ...
          logger.warn(
            Colors.warning(
              `WARNING: Error changing alias but unable to delete new index ${newIndex}`
            )
          )
        }
        throw error
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            `CRITICAL ERROR: Error loading new index '${newIndex}'` +
      ...
      try {
        ...
        throw new Error(
          `CRITICAL ERROR: The new index '${newIndex}'` +
      ...
      await Promise.all(
        readIndices.map(async index => {
          try {
            await esapi.deleteIndex(index)
          } catch (ignoreError) {
            logger.info(
              Colors.warning(
                `WARNING: New index created and loaded successfully but failed to delete old index: ${index}`
              )
            )
          }
        })
      ...
      logger.info(
        Colors.info(
          `New index '${chalk.bold(
            newIndex
          )}' created successfully, documents indexed, and read and write aliases pointed to new index.`
        )
      )
      return newIndex
    })
...
async function reindexAction(index, options, config) {
  try {
    ...
    logger.info(
      Colors.prep(
        `About to reindex into a new index for indices ${indicesMsg}.`
      )
    )
    const confirmed =
      options.bypassConfirm || (await confirm(Colors.warning(`Are you sure?`)))
  ...
  return Promise.all(
    indices.map(async indexConfig => {
      ...
      try {
        if (verbose) {
          logger.info(
            Colors.info(`Reindexing from ${index}_read to ${index}_write`)
          )
        }
        ...
        try {
          ...
          throw new Error(
            `CRITICAL ERROR: Error reindexing to new index '${newIndex}'` +
              ` and unable to change write alias back to original index: '${index}.` +
              ` Index is in inconsistent state'.` +
              ` Original error from loading new index: ${error.message}`
          )
      ...
      await Promise.all(
        readIndices.map(async index => {
          try {
            ...
            logger.warn(
              Colors.warning(
                `WARNING: Reindex success but failed to delete old index: ${index}`
              )
            )
          }
        })
      ...
      logger.info(
        Colors.info(
          `New index '${chalk.bold(
            newIndex
          )}' created successfully, documents reindexed, and read and write aliases pointed to new index.`
        )
      )
      return newIndex
    })
