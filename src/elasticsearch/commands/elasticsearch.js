...
exports.addCommand = (program, config) => {
  ...
  .action((index, options) => esAction(config, () => loadIndex.loadIndexAction(index, options, config)))
  ...
  .action((index, options) => esAction(config, () => updateIndex.updateIndexReloadAction(index, options, config)))

  program.command('es:reindex [index]')
  .alias('reindex')
  .description('Create a new index using the defined mapping and reindexes all documents for the index')
  .option('-y, --bypassConfirm', 'Bypasses confirmation prompt. Required when non-interactive stdout.')
  .option('-v, --verbose', 'Displays additional progress information.')
  .action((index, options) => esAction(config, () => updateIndex.reindexAction(index, options, config)))

  program.command('es:search <text> [index]')
  .alias('search')
  ...
  .action((text, index, options) => esAction(config, () => searchIndex.searchIndexAction(text, index, options, config)))
}
