...
const backupAction = async (docSetId, options, config) => {
  try {
    ...
    const backupBasePath =
      options.basePath ||
      (config.has('firestore.backupBasePath')
        ? config.get('firestore.backupBasePath')
        : null) ||
      './backups'
    ...
    if (confirmed) {
      ...
      const visit = doc =>
        backup(doc, backupPath, verbose, file => files.push(file))
      const batchOptions = {
        visit
      }
      ...
      logger.info(
        Colors.complete(
          `Completed backup of ${traverseBatch.progressBar.curr} documents`
        )
      )
    }
  } catch (error) {
